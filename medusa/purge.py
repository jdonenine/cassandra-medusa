...
def purge_backups(storage, backups, backup_grace_period_in_days):
    ...
    for fqdn in fqdns:
        (cleaned_objects_count, cleaned_objects_size) = cleanup_obsolete_files(storage, fqdn, backup_grace_period_in_days)
...
def filter_files_within_gc_grace(storage_driver, blobs, backup_grace_period_in_days):
    return list(
            filter(lambda blob: 
                is_older_than_gc_grace(storage_driver.get_object_datetime(blob), backup_grace_period_in_days)
                , blobs))

def is_older_than_gc_grace(blob_datetime, gc_grace) -> bool:
    return datetime.timestamp(blob_datetime) <= datetime.timestamp(datetime.now()) - (gc_grace * 86400)
